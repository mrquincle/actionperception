  ...
  using namespace boost::numeric::ublas;
    ...
    void inplace_solve (const matrix_expression<E1> &e1, vector_expression<E2> &e2,
      ...
      std::cout << " is_lc ";
        ...
        BOOST_boost::numeric::ublas_CHECK (e1 ().size1 () == e1 ().size2 (), bad_size ());
        BOOST_boost::numeric::ublas_CHECK (e1 ().size2 () == e2 ().size (), bad_size ());
        size_type size = e2 ().size ();
        for (size_type n = 0; n < size; ++ n) {
#ifndef BOOST_boost::numeric::ublas_SINGULAR_CHECK
            BOOST_boost::numeric::ublas_CHECK (e1 () (n, n) != value_type/*zero*/(), singular ());
#else
...
void
cholesky_solve(const TRIA& L, VEC& x, boost::numeric::ublas::lower)
{
  using namespace boost::numeric::ublas;
